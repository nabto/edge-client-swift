 ...
 * callbacks. It is up to the application to make the machinery that
 ...
 *
 * @param context [in]  The context
 * @retval NABTO_CLIENT_EC_OK if the context is freed
 ...
 *
 * This function is blocking until no more callbacks are in progress on the event
 * or callback queues.
 ...
 * @retval NABTO_CLIENT_EC_OK if the system was stopped.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 * @retval NABTO_CLIENT_EC_OK if the private key is created and available in privateKey.
 ...
 * ```
 *
 * Use pre 5.2 local discovery.  Before 5.2 devices are located by
 * doing an mDNS scan for all the devices. After 5.2 including 5.2,
 ...
 * @retval NABTO_CLIENT_EC_OK  if the json document is parsed and understood.
 ...
 * @return NABTO_CLIENT_EC_OK if the options are present in the output string.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 * server api key will be put into the final applications it is not
 * secret.
 *
 * @param connection [in] the connection
 ...
 * that it is easier to understand what apps has what communicaton
 ...
 * @retval NABTO_CLIENT_EC_INVALID_ARGUMENT if the arguments are obviously invalid. e.g. using port number 0
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 * errors which are not supposed to happen if the software is used
 ...
 * specified is invalid, then it is logged as an error.
 *
 * Future status:
 *  - NABTO_CLIENT_EC_OK if connection is ok and connected to
 *    the device.
 ...
 *
 * UdpRelayError, set a remote udp relay was tried, it is ok if udp
 * relay was created.
 *
...
/**
 * Connection events. Connection events are used to notify an
 ...
 * @return Returns NABTO_CLIENT_OK if the payload and content format were successfully set.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 * @retval NABTO_DEVICE_EC_OK if response has a contentFormat
 ...
 *                        number is 0, the SDK will choose the port number.
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API
...
typedef enum NabtoClientMdnsAction_ {
    /*
     * This action is emitted when an mDNS cache item is added.
     */
 ...
 *
 * Init an mDNS result listener. If the subtype is non null or the non
 ...
 * [Futures Guide](/developer/platforms/embedded/nabto_futures.html).
 *
 ...
 * @param callback [in]  The callback.
 * @param data [in]  User data for the callback.
 * @return NABTO_CLIENT_EC_OK if the callback is set
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 * it is not allowed to call any nabto_client_* functions from the log
 * callback as that would result in a deadlock. If it is needed to
 ...
 * @retval NABTO_CLIENT_EC_OK if successfully set
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
